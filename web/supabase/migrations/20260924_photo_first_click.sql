-- First eligible drawer acquisition per KST band is guaranteed; later attempts remain 35%.
...
begin
 ...
 if p_action<>'wardrobe' and not(p_action='drawer' and coalesce((r.daily_photo_category_counts->>('drawer:'||band))::integer,0)=0) and coalesce((r.reward_receipts->>'photo_last_event_at')::timestamptz,'epoch') > clock_timestamp()-interval '3 seconds' then return empty_result; end if;
 ...
 foreach category in array categories loop
  ...
  if selected is null or (category<>'outfit' and not(p_action='drawer' and coalesce((r.daily_photo_category_counts->>('drawer:'||band))::integer,0)=0) and random()>=coalesce((p_chances->>category)::numeric,0)) then continue; end if;
  ...
  if p_action='drawer' then update public.user_progression set daily_photo_category_counts=daily_photo_category_counts||jsonb_build_object('drawer:'||band,1) where user_id=p_user_id; end if;
