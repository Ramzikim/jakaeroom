-- Photo prop rules; preserve existing collections and receipts.
...
begin
 if p_event_id is null or p_action is null or p_action not in ('pet','hug','food','wardrobe','drawer','window','tv','game','cushion','walk','idle','bath','sleep') then raise exception 'Invalid photo event'; end if;
 ...
 if p_action<>'wardrobe' and coalesce((r.reward_receipts->>'photo_last_event_at')::timestamptz,'epoch') > clock_timestamp()-interval '3 seconds' then return empty_result; end if;
 ...
 categories:=case p_action when 'food' then array['food','happy'] when 'wardrobe' then array['outfit'] when 'window' then array['window','happy'] else array['happy'] end;
 foreach category in array categories loop
  cap:=case category when 'food' then 1 when 'happy' then 6 else 2 end;
  ...
  if selected is null or (category<>'outfit' and random()>=coalesce((p_chances->>category)::numeric,0)) then continue; end if;
...
-- Only the development HTTP route can request this using a verified account.
create or replace function public.reset_progression_dev(p_user_id uuid,p_scope text)
returns void language plpgsql security definer set search_path='' as $$
declare r public.user_progression;
begin
 if p_user_id is null or p_scope is null or p_scope not in ('letters','collection','coins') then raise exception 'Invalid reset'; end if;
 perform public.mutate_progression(p_user_id,'ensure');
 select * into r from public.user_progression where user_id=p_user_id for update;
 if p_scope='coins' then
  update public.user_progression set heart_coin_balance=0 where user_id=p_user_id;
 else
  update public.user_progression set collected_letter_ids='{}',
   letter_state=jsonb_build_object('version',coalesce((r.letter_state->>'version')::integer,0)+1,'date','','counts','{}'::jsonb,'vipIds','[]'::jsonb),
   owned_gift_ids=array_remove(owned_gift_ids,'starlight_mailbox') where user_id=p_user_id;
  if p_scope='collection' then
   update public.user_progression set collected_photo_ids='{}',daily_photo_total=0,daily_photo_category_counts='{}',
    reward_receipts=reward_receipts-'photo_last_event_at' where user_id=p_user_id;
  end if;
 end if;
end $$;
revoke all on function public.reset_progression_dev(uuid,text) from public,anon,authenticated;
grant execute on function public.reset_progression_dev(uuid,text) to service_role;
