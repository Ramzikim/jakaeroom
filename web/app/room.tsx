...
export default function Room(){
 ...
 useEffect(()=>{
  ...
  try{doc.modelContext.registerTool({name:'play_with_jakae',description:'Pet or wake Jakae, sleep, bathe, eat a strawberry, or receive a limited letter.',inputSchema:{type:'object',properties:{action:{type:'string',enum:['pet','bed','bath','berry','letter']}},required:['action'],additionalProperties:false},execute:(input:unknown)=>{const value=input as {action?:string};if(!value||!['pet','bed','bath','berry','letter'].includes(value.action||''))throw new Error('Invalid action');if(value.action==='letter')return {accepted:openLetter()};act(value.action as Action);return {accepted:true};}}, {signal:abort.signal});}catch{/* Optional browser API. */}
  return()=>abort.abort();
 },[ready]);
