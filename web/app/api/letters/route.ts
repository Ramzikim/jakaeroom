...
export async function POST(request:Request){
 ...
 let input:{action:LetterAction;eventId:string;letterId?:string};
 ...
 try{const db=createClient(process.env.NEXT_PUBLIC_SUPABASE_URL!,process.env.SUPABASE_SERVICE_ROLE_KEY!,{auth:{persistSession:false,autoRefreshToken:false}});
  ...
  return json(await letterEvent(user.id,input.action,['load','awake','sleep'].includes(input.action)?'':input.eventId,input.letterId));
