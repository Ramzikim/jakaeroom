...
import {classifyVip,makeProfile,parseProfile} from '../../profile';
...
async function handle(request:Request){
 ...
 try{
  ...
  const profile=makeProfile(input,user.id,tier,existing?.createdAt);
  const {error:saveError}=await db.from('profiles').upsert(profile,{onConflict:'userId',ignoreDuplicates:migration});
  if(saveError)throw saveError;
  if(migration){const {data,error}=await db.from('profiles').select('*').eq('userId',user.id).single();if(error)throw error;return json({profile:data});}
  return json({profile});
