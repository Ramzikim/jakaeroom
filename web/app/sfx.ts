...
let context:AudioContext|undefined,master:GainNode|undefined,enabled=false,volume=.8,active=0;
...
export async function unlockSfx(){
 ...
 try{const Audio=window.AudioContext||(window as Window & {webkitAudioContext?:typeof AudioContext}).webkitAudioContext;if(!Audio)return false;
 if(!context||context.state==='closed'){context=new Audio();master=undefined;rustle=undefined;last.clear();}
 if(!master){master=context.createGain();master.gain.value=enabled?volume:0;master.connect(context.destination);}
 if(context.state!=='running')await context.resume();return context.state==='running';}catch{return false;}
}
...
export function playSfx(kind:Sfx,alternate=0){
 ...
 envelope.gain.setValueAtTime(0,now);envelope.gain.linearRampToValueAtTime(gain*1.8,now+.008);envelope.gain.setValueAtTime(gain*1.8,now+duration*.35);envelope.gain.exponentialRampToValueAtTime(.0001,now+duration);
