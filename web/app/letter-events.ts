...
export async function requestLetterEvent(action:LetterAction,letterId?:string,origin?:CoinOrigin):Promise<LetterEventResult|null>{
 const client=authClient(),identity=client?.auth.getSession(),eventId=crypto.randomUUID();let output:LetterEventResult|null=null;
 await queueCoinOperation(async()=>{
  const session=(await identity)?.data.session;
  if(!session){
   ...
   const next=advanceLetters(state,saved?.collected??[],action,Date.now(),'normal',letterId);
  ...
  const response=await fetch('/api/letters',{method:'POST',headers:{Authorization:`Bearer ${current.access_token}`,'Content-Type':'application/json'},body:JSON.stringify({action,eventId,letterId})});
  if(!response.ok)throw Error('Letter event unavailable');
  const result:LetterResult=await response.json();
