...
export async function requestLetterEvent(action:LetterAction,letterId?:string,origin?:CoinOrigin,retryEventId?:string):Promise<LetterEventResult|null>{
 const client=authClient(),identity=client?.auth.getSession(),eventId=retryEventId??crypto.randomUUID(),clickedAt=Date.now();let output:LetterEventResult|null=null;
 await queueCoinOperation(async()=>{
  ...
  if(!session){
   ...
   output={userId:null,letterId:next.letterId,reason:next.reason,state:next.state,collected:next.owned};return;
  }
