...
const sitLines=['여기 앉아 있으니까 편하다아.','뭐하고 놀까아?','조금 심심해애.','딸기 생각나아...','가만히 있는 것도 좋아아.','누가 놀러 안 오나아.','잠깐 쉬는 중이야아.','작애 지금 멍때리고 있어어.'];
const dawnLines=['졸린데 잠이 안와아...','조금만 더 놀다가 잘래애.','눈은 감기는데에... 아직 안 잘래애.','새벽은 조용해서 좋은데에, 조금 심심해애.','잠깐만 돌아다니다가 다시 잘 거야아.','자야 하는데에... 괜히 놀고 싶어어.','꿈꾸기 전에 조금만 더 있을래애.'];
const dreamLines=['딸기 케이크 먹는 꿈 꾸는 중이야아...','엄청 신나는 꿈 꾸고 있어어!','미미랑 놀러 가는 꿈이야아.','딸기가 산만큼 나왔어어...','재밌는 꿈 꾸는 중이야아.','으음... 조금 슬픈 꿈이야아...','구름 위에서 뛰어노는 꿈이야아.','과자 잔뜩 먹는 꿈 꾸고 있어어.','쭈욱 자고 싶다아...','새근새근...'];
const refusalLines=['나중에 쓰다듬어어!','털 다 망가지겠어!','작애 털 눌리잖아아.','잠깐만 쉬었다가아!'];
...
export function createLife(){return {sequence:'idle' as Sequence,started:0,now:0,wait:5,node:'center',destination:'center',path:[] as Point[],point:[1.65,0,-.55],pending:null as null|'sit'|'sleep',seat:'bed' as 'bed'|'rug',speech:'왔어어? 내 방에서 같이 놀자!',speechUntil:7,nextSpeech:0,pets:[] as number[],awakeUntil:0,wetUntil:0};}
...
function arrive(s:Life,rng:()=>number){s.node=s.destination;if(s.pending){const sleep=s.pending==='sleep';place(s,s.seat,s.seat);change(s,sleep?'sit_snooze':'sit_idle');s.wait=12+rng()*23;s.nextSpeech=s.now+15+rng()*12;s.speech='';if(!sleep&&rng()<.5)say(s,pick(sitLines,rng));}else{idle(s,rng);if(rng()<.65)say(s,pick(waitingLines,rng));}}
export function commandLife(s:Life,action:Action,rng=Math.random):boolean{
 ...
 s.pets.push(s.now);change(s,rng()<.5?'hop':'shy');say(s,'히히, 나 귀엽지이?');return true;
}
export function tickLife(s:Life,dt:number,band:Band,lengths:Record<Sequence,number>,rng=Math.random){
 ...
 else if(s.sequence==='bath'){if(elapsed>=RULES.bathSeconds){place(s,'bathExit','bathExit');s.wetUntil=s.now+5;go(s,'center');say(s,pick(wetLines,rng));}}
