...
import {publishCoins,queueCoinOperation} from './coin-events';
import {GIFT_REGISTRY,GIFT_COPY,shoppingUnlocked,type Gift} from '../lib/gifts';
import {useProgression,refreshProgression} from './progression-store';
...
export function GiftProvider({children}:{children:ReactNode}){
 const {progression,userId}=useProgression();
 const [cabinet,setCabinet]=useState(false),[confirmation,setConfirmation]=useState<{gift:Gift;band?:Band}|null>(null),[pending,setPending]=useState(false),[error,setError]=useState(''),[notice,setNotice]=useState(false);
 const user=useRef<string|null>(null),busy=useRef(false);
 useEffect(()=>{user.current=userId;setConfirmation(null);setNotice(false);setError('');},[userId]);
 ...
 async function purchase(){
  ...
  const selected=confirmation,buyer=user.current;
  await queueCoinOperation(async()=>{
   try{
    const session=(await withTimeout(authClient()?.auth.getSession()))?.data.session;if(!session){setError('로그인 후 선물할 수 있어요.');return;}if(session.user.id!==buyer)return;
    const result=await requestJson('/api/gifts',{method:'POST',headers:{Authorization:`Bearer ${session.access_token}`,'Content-Type':'application/json'},body:JSON.stringify({giftId:selected.gift.id,route:selected.gift.acquisitionType,band:selected.band})});
    if(user.current!==session.user.id)return;
    ...
    if(!result.progression){void refreshProgression(true);}
   ...
   }catch{setError('처리 결과를 확인하지 못했어요. 잠시 후 보유 상태를 확인해 주세요.');void refreshProgression(true);}
