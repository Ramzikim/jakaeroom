...
VIP는 프로필의 공백 제거한 이름(trim만), 성별, 출생연도 정확 일치로 계산하는 대사·호칭 전용 값입니다. UUID·이메일과 무관하며 권한을 부여하지 않습니다. 기존 DB는 배포 전 supabase/profile-vip-migration.sql을 실행합니다.
...
5. 위 환경 변수 세 개를 설정하고 앱을 재시작/재배포합니다.
...
- 호칭은 KST 연도로 계산하고 서버 조회/저장 시 갱신합니다. 프로필을 편집할 때 VIP도 다시 계산합니다.
