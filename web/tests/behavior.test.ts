...
for(const r of [.1,.9]){const s=createLife();s.node=r<.5?'bed':'rug';s.point=[...positions[r<.5?'bedRight':'rug']];s.wait=0;let calls=0;tickLife(s,.1,'night',lengths,()=>calls++===0?.2:r);assert.equal(s.sequence,'sit_idle');assert.deepEqual(s.point,positions[r<.5?'bed':'rug']);}
