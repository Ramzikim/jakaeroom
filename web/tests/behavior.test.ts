...
const spam=createLife();for(let i=0;i<4;i++){commandLife(spam,'pet',()=>.1);advance(spam,1.1);}commandLife(spam,'pet',()=>.1);assert.equal(spam.sequence,'idle');assert.match(spam.speech,/쓰다듬|털|쉬었다/);
