...
for(const band of ['day','afternoon','night','dawn'] as const){
 ...
 advance(s,2.9);assert.equal(s.speech,'');advance(s,.2);assert(TV_CONTENT[band].some(v=>v.script===s.mediaNews&&v.reaction===s.speech));assert(!commandLife(s,'tv'));advance(s,7);assert.equal(s.media,null);assert(s.sequence.includes('walk'));
}
const s=createLife();
...
for(const band of ['day','afternoon','night','dawn'] as const)for(let i=0;i<TV_CONTENT[band].length;i++){
 ...
 assert.equal(state.mediaTitle,TV_CONTENT[band][i].title);assert.equal(state.mediaNews,TV_CONTENT[band][i].script);advance(state,3.1);assert.equal(state.speech,TV_CONTENT[band][i].reaction);
}
