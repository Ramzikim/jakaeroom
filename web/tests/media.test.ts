...
import {TV_NEWS,GAME_DIALOGUE,GAME_LIMIT_DIALOGUE} from '../app/media-dialogue.ts';
...
for(const band of ['day','afternoon','night','dawn'] as const){
 const s=createLife();assert(commandLife(s,'tv',()=>.5,undefined,band));assert.equal(s.node,'cushion');assert.equal(s.sequence,'sit_idle');assert(TV_NEWS[band].some(v=>v.news===s.mediaNews));assert.equal(s.speech,'');
 advance(s,2.9);assert.equal(s.speech,'');advance(s,.2);assert(TV_NEWS[band].some(v=>v.news===s.mediaNews&&v.reaction===s.speech));assert(!commandLife(s,'tv'));advance(s,7);assert.equal(s.media,null);assert(s.sequence.includes('walk'));
}
const s=createLife();
...
console.log('TV bands, 3s reaction, 10s exit, game 8s exit, 3-per-band limit, duplicate clicks and interruptions passed.');
for(const band of ['day','afternoon','night','dawn'] as const)for(let i=0;i<3;i++){
 const state=createLife();commandLife(state,'tv',()=>i/3,undefined,band);
 assert.equal(state.mediaNews,TV_NEWS[band][i].news);advance(state,3.1);assert.equal(state.speech,TV_NEWS[band][i].reaction);
}
...
console.log('All 12 news/reaction pairs and all 3 monitor dialogue mappings passed.');
